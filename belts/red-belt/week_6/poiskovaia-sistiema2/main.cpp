...
void TestStress() {
  ...
  size_t max_documents = 800; // 50'000
  size_t max_queries = 300'000; // 500'000
  size_t max_unique_words = 10'000; // 10'000
  size_t min_word_length = 3;
  size_t max_words_per_query = 10; // 10
  size_t max_word_length = 100; // 100
  size_t max_words_per_doc = 1'000; // 1'000
  vector<string> docs(max_documents);
  set<string> unique_words_set;

  for (string line; unique_words_set.size() < max_unique_words && getline(ifs, line);) {
    transform(begin(line), end(line), begin(line), [](unsigned char c) {
      return isalpha(c) ? tolower(c) : ' ';
      });
    for (const auto& word_view : SplitBy(line, ' ')) {
      if (unique_words_set.size() >= max_unique_words)
        break;
      if (word_view.size() < min_word_length || word_view.size() > max_word_length)
        continue;
  ...
  ifs.close();
  ASSERT_EQUAL(unique_words_set.size(), max_unique_words);
  ...
  random_device rd;
  mt19937 gen(rd());
  uniform_int_distribution<size_t> dis(0, max_unique_words - 1);
  for (size_t i = 0; i < max_documents; ++i) {

    docs[i].reserve(max_words_per_doc * (min_word_length + max_word_length) / 2);

    for (size_t j = 0; j < max_words_per_doc; ++j) {
      size_t index = dis(gen);
      (docs[i] += ' ') += unique_words_vector[index];
    }
  }

  vector<string> queries(max_queries);
  for (size_t i = 0; i < max_queries; ++i) {
    for (size_t j = 0; j < max_words_per_query; ++j) {
      size_t index = dis(gen);
