...
void TestStress() {
  ...
  size_t max_documents = 50'000; // 50'000
  size_t max_queries = 500'000; // 500'000
  ...
  for (string input; getline(ifs, input) && docs.size() < max_documents; ) {
    ...
    for (const auto& word_view : SplitIntoWords(input)) {
      string word(word_view);
      if (word_view.size() < min_word_length || word.size() > max_word_length)
        continue;
  ...
  {
    cerr << "# documents count: " << docs.size() << " ("
      << (docs.size() * 100. / max_documents) << "%)" << endl;
    cerr << "# unique words count: " << unique_words.size() << " ("
      << (unique_words.size() * 100. / max_unique_words) << "%)" << endl;
    cerr << "# queries count: " << queries.size() << " ("
      << (queries.size() * 100. / max_queries) << "%)" << endl;
  }
