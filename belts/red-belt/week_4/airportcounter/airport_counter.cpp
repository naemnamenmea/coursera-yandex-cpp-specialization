...
public:
	...
	void EraseOne(TAirport airport) {
		auto& count = GetCountRef(airport);
		if (count > 0)
			--count;
	}

	void EraseAll(TAirport airport) {
		GetCountRef(airport) = 0;
	}

	const static size_t SIZE = static_cast<size_t>(TAirport::Last_);
...
private:
	...
	size_t& GetCountRef(TAirport airport) {
		return data[static_cast<size_t>(airport)].second;
	}
};
...
int main() {
	...
	ifstream ifs(INPUT_FILENAME); cin.rdbuf(ifs.rdbuf());
#endif // LOCAL_KONTROL
