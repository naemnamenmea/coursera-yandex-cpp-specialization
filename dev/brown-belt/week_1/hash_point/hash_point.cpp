#if not defined LOCAL_LAUNCH or defined SAHDGSASDFFAGSHDFASDASD
...
struct Hasher {
  size_t operator()(const Point3D& point) const
  {
    ...
    size_t p = 2037;
    size_t a = hasher(point.x);
    size_t b = hasher(point.y);
    size_t c = hasher(point.z);

    return hasher(a * p * p + b * p + c);
  }
};
