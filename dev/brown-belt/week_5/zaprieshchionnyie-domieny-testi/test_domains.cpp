...
void TestSimple() {
  // Your tests here
  {// test 1
    string input = "abra.box";
    vector<string_view> res = Split(input, ".");
    ASSERT(res.size() > 0);
    ASSERT(!res.front().empty());
  }
  {// test 2
    string input = "abra.box";
    Domain domain(input);
    ...
    vector<string> reversedParts(begin(range),end(range));
    reverse(begin(reversedParts), end(reversedParts));
    ASSERT(equal(begin(parts), end(parts), begin(reversedParts)));
  }
  {// test 3
    string input = "abra.box";
    Domain domain(input);
    ASSERT(::IsSubOrSuperDomain(domain, domain));
  }
  {// test 4
    Domain domain("abra.box");
    Domain subDomain("ru.abra.box");
    ASSERT(::IsSubdomain(subDomain, domain));
  }
  {// test 5
    const vector<Domain> v_test{ Domain("test.com"), Domain("a.test.com") };
    DomainChecker domainChecker(v_test.begin(), v_test.end());
    ASSERT(domainChecker.IsSubdomain(Domain("b.test.com")));
  }
  {// test 6
    vector<Domain> bannedDomains = { Domain{"mail.ru"} };
    DomainChecker domainChecker(begin(bannedDomains),end(bannedDomains));
    ASSERT(!domainChecker.IsSubdomain(Domain{ "abra.com" }));
  }
  {// test 7
    vector<Domain> bannedDomains = { Domain{"mail.ru"} };
    vector<Domain> domainsToCheck = { Domain{"abra.com"} };
    vector<bool> res = CheckDomains(bannedDomains, domainsToCheck);
    stringstream ss;
    PrintCheckResults(res, ss);
    string output;
    ss >> output;
    ASSERT(output.find("Bad") == output.npos);
  }
  {// test 8
    stringstream iss;
    iss << "4\n"
    ...
    ASSERT(!banned_domains.empty() && banned_domains.front().GetPartCount() != 0);
  }
}
