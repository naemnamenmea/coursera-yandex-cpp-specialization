...
const unordered_map<HttpCode, string> HttpCodeComment =
{
  {HttpCode::Ok, "OK"},
...
public:
  ...
  {
    output << "HTTP/1.1 " << static_cast<unsigned int>(resp.m_code) << ' ' << HttpCodeComment.at(resp.m_code) << '\n';
  ...
  HttpResponse ServeRequest(const HttpRequest& req) {
    ...
    if (req.method == "POST") {
      if (req.path == "/add_user") {
        ...
        httpResponse.SetCode(HttpCode::Ok).SetContent(move(response));
      }
      ...
      if (req.path == "/user_comments") {
        ...
        httpResponse.SetCode(HttpCode::Ok).SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
      }
      else {
        httpResponse.SetCode(HttpCode::NotFound);
      }
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
  HttpResponse rawResponse = srv.ServeRequest(request);
  ParsedResponse resp;
  stringstream ss;
  ss << rawResponse;
  ss >> resp;
