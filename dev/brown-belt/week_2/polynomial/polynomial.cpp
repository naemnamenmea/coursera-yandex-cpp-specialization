...
public:
  ...
  struct CoeffHolder
  {
    CoeffHolder(Polynomial<T>& poly, size_t degree) : m_poly(poly), m_degree(degree) {}

    CoeffHolder& operator=(const T& coeff)
    {
      if (m_degree > m_poly.Degree())
      {
        if (coeff != 0)
        {
          vector<T> coeffVec(m_degree + 1, 0);
          coeffVec[m_degree] = coeff;
          Polynomial<T> poly(std::move(coeffVec));
          poly += m_poly;
          std::swap(poly, m_poly);
        }
      }
      else
      {
        m_poly.coeffs_[m_degree] = coeff;
        if (coeff == 0) m_poly.Shrink();
      }
      return *this;
    }

    operator T () const {
      return m_degree > m_poly.Degree() ? 0 : m_poly.coeffs_[m_degree];
    }
  ...
  CoeffHolder operator[](size_t degree)
  {
    return CoeffHolder(*this, degree);
  }
